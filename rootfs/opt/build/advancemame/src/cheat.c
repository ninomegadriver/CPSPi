...
static mame_timer			* dispenser_timer;
...
static void 	dispenser_periodic(int param);
...
void cheat_init(void)
{
	...
	dispenser_timer = timer_alloc(dispenser_periodic);
	timer_adjust(dispenser_timer, TIME_IN_HZ(Machine->refresh_rate), 0, TIME_IN_HZ(Machine->refresh_rate));
...
int card_dispenser_lock = 0;
int card_dispenser_release = 0;
int dispensed_cards = 0;
void dispenser_periodic(int param){

//UINT32 tmp = DoCPURead(0, 0xFFA4C4, 2, 0);
//printf("DBG: 0x%08x\n", tmp);

	if (
		strstr(game_name, "sf2ce") != NULL ||
		strstr(game_name, "sf2t") != NULL ||
		strstr(game_name, "sf2r") != NULL ||
		strstr(game_name, "sf2m") != NULL
	) {
		UINT32 stage = DoCPURead(0, 0xFF89ED, 2, 0);
		if(stage == 0x0c03 && card_dispenser_lock == 0){ // Reached the End Of The Game
			card_dispenser_lock = 1;
			card_dispenser_release = 1;
		}else if (stage == 0x00){ // Game Start, First Stage
			card_dispenser_lock = 0;
			card_dispenser_release = 0;
		}
  }else if (strstr(game_name, "sf2") != NULL) {
			UINT32 stage = DoCPURead(0, 0xFF89F9, 2, 0);
			if(stage == 0x0b03 && card_dispenser_lock == 0){ // Reached the End Of The Game
				card_dispenser_lock = 1;
				card_dispenser_release = 1;
			}else if (stage == 0x00){ // Game Start, First Stage
				card_dispenser_lock = 0;
	...
	}else if (strstr(game_name, "knights") != NULL) {
		UINT32 stage = DoCPURead(0, 0xFFA4C4, 2, 0);
		if(stage == 0x606 && card_dispenser_lock == 0){ // Reached the End Of The Game
			card_dispenser_lock = 1;
			card_dispenser_release = 1;
		}else if (stage == 0x00){ // Game Start, First Stage
			card_dispenser_lock = 0;
	...
	}else if (strstr(game_name, "kod") != NULL) {
		UINT32 stage = DoCPURead(0, 0xFF0E49, 1, 0);
		if(stage == 0x0f && card_dispenser_lock == 0){ // Reached the End Of The Game
			card_dispenser_lock = 1;
			card_dispenser_release = 1;
		}else if (stage == 0x00){ // Game Start, First Stage
			card_dispenser_lock = 0;
	...
	// If Conditions met, give out the card
	if(card_dispenser_release > 0){
		//ui_popup_time(1, "You've won a card...");
		digitalWrite(25, HIGH);
		card_dispenser_release++;
		if(card_dispenser_release>180){
			card_dispenser_release = 0;
			digitalWrite(25, LOW);
		}
	}
...
static void WriteData(CheatAction * action, UINT32 data)
{
	...
	switch(EXTRACT_FIELD(action->type, LocationType))
	{
		case kLocation_Standard:
		{
			...
			//printf("NINO: DoCPUWrite(UINT32 0x%08x, UINT8 0x%08x, UINT32 0x%08x, UINT8 0x%08x, UINT8 0x%08x);\n",data, parameter, action->address, bytes, CPUNeedsSwap(parameter) ^ swapBytes);
		}
		break;
		...
		case kLocation_MemoryRegion:
		{
			int		region = REGION_CPU1 + parameter;
			UINT8	* buf = memory_region(region);
			//printf("NINO: kLocation_MemoryRegion\n");
		...
		case kLocation_HandlerMemory:
		{
			UINT32	relativeAddress;
			UINT8	** buf;
			//printf("NINO: kLocation_HandlerMemory\n");
			if(!action->cachedPointer)
			{
				action->cachedPointer = LookupHandlerMemory(parameter, action->address, &action->cachedOffset);
			}
		...
		case kLocation_IndirectIndexed:
		{
			...
			printf("NINO: kLocation_IndirectIndexed\n");
